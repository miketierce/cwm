...
|------|-----------|-------------|
| **G1 — Hypothesis registration** | All hypotheses written as falsifiable H-statements with kill criteria *before* writing code | This document |
...
**Running totals:** 29 modules · 680 tests · test count must only go up.
...
|------|------|----------|--------|
| C-1 | Review `helmholtz_2d.py` — assess whether it can be extended to plate eigenproblems or if a new solver is needed | Design notes | ⬜ |
| C-2 | Implement `simulations/chladni_plates.py` with 4 experiment functions + dataclass results | Module | ⬜ |
| C-3 | Write `tests/test_chladni_plates.py` — target ≥ 40 tests | Test file | ⬜ |
| C-4 | Run experiments, tune parameters until all 4 confirm or are honestly killed | Results | ⬜ |
| C-5 | Update `simulations/__init__.py` (Phase 9a) | Package | ⬜ |
| C-6 | Add §11.8 or new subsection to paper (if confirmed) + §14.2 historical bullet | Paper | ⬜ |
| C-7 | Run full test suite — must exceed 680 with zero failures | Regression | ⬜ |
| C-8 | Regenerate both PDFs | Deliverable | ⬜ |

### External data sources (for validation, not curve-fitting)

- Chladni's original frequency tables (1787, *Entdeckungen über die Natur des Klanges*)
