...
> **STATUS 2026-06-21 — OPTIMIZER BUILT + RUN ([tools/array_design.py](../tools/array_design.py)).** _Corrected 2026-06-22 after user challenge — recomputed, see below._ **USE ALL 8 PIO TONES.** The earlier "3 plates / 6 tones" answer was an artifact of an over-harsh per-tone cost penalty plus a collision model that subtracted overlap bins as waste — but [tools/collision_value_test.py](../tools/collision_value_test.py) MEASURED that collision bins carry _higher_ mean SNR than the kept pile, so for a learned/kernel readout (Pong/DOOM) they are features, not waste. With `collision_value` per regime, multipoint `--max-tx`, and realistic (cheap) tones, **every regime now maxes the 8 PIO tones**: ATTRIBUTION → 100 mm + slide*A + slide_C with **3 TX per slide** (≈54 usable, 7 clean); **KERNEL (collisions = features) → 100 mm + all 3 slides, 2 TX each (≈63 modes, 28 collisions kept)** — the 4th plate the attribution model discarded comes back. A 15-tone PWM-firmware build (8 PIO + 7 PWM, flashed v2.0) scales to all 5 plates / ≈84 modes. **Max-TX physics (10 mm brass PZTs on the 22 g 100 mm plate):** area ~50–120 and mass (~15–30) are not binding; damping is soft-binding at ~8–12 (loaded Q≈Q₁/N, ~Q250 at 8 PZTs); the \_useful* cap is **modal independence ~4–6 TX/plate** (excitation vectors ≤ #resolvable modes; flexural λ≈10 mm at 100 kHz → TX ≥10–16 mm apart); the _hard_ cap is the **8 tone generators** shared across all plates. So lean into multipoint (2–3 TX/plate everywhere) but spend tone #7–8 on a new band-separated plate over a 5th TX. **Long TX wire: thinner marginally beats thicker** (capacitive load → series R irrelevant; thicker = more parasitic shunt C), but the real lever is a **twisted pair / coax** (kills the radiated EMI feedthrough that was the MEASURED 8–260× SNR loss) — give the shortest run to the highest-frequency plate. Wire-length remains the #1 SNR driver. Firmware: two symmetric phase-locked PIO quads (Block A PIO0=CH1-4/PH1-4, Block B PIO1=CH5-8/PH5-8 — each holds up to 4 TX in phase) plus a 7-tone PWM block (CH9-15); flashed v2.0 2026-06-22. Open step = install → per-plate solo censuses → re-run the optimizer on real catalogs (`data/results/array_design/catalogs/`). Full write-up in `docs/lab_diary_20260620.md` (2026-06-22 correction entry).
...
- **+7 PWM slices** — 7 of the 8 PWM slices can each emit one more independent frequency. (Slice 4 routes only to GP8/GP9 — used by PIO Block B — or GP24/GP25, which are not on the Pico header, so it is unavailable; and the two channels of a slice share a counter, so it is one frequency per slice.) **Firmware v2.0 (flashed 2026-06-22, [tools/pico_nco/main.py](../tools/pico_nco/main.py)) implements this: 8 PIO + 7 PWM = 15 independent tones in 3 phase-coherent blocks** (two PIO quads of 4 that hold a multipoint plate's TX in phase, + 7 sync-started PWM tones for simple plates). PWM frequency resolution is coarser at the top of the band (fine for low/mid-band plates). See the color-coded pin map: [companion/nco_pinmap.html](../companion/nco_pinmap.html).
...
Per candidate plate `p`, from an individual full-band census: mode set `M_p = {(f_i, SNR_i, Q_i)}`. The decision variable is a **grouping**: a set of drive channels, each assigned one-or-more plates (broadcast) or one plate assigned one-or-more channels (multi-point), using **up to** the available tone budget (8 now / 15 with PWM). On the shared RX bus a mode survives only if no other _active_ mode lies within `collision_bw`. Broadcast groups also apply an SNR penalty for the parallel-load amplitude drop. Maximize a **quality-weighted feature score** for the whole array:
