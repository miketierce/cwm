#!/usr/bin/env python3
"""Capture a live tap on Rod 1 and compare using frequency-matching."""
...
live_arr = np.array(live_hz)
print(f"\n  Live peaks ({len(live_arr)} found):")
for i, h in enumerate(live_arr):
    print(f"    [{i+1:2d}] {h:10.1f} Hz")

# Load enrolled
db = json.loads(open("data/results/lab/users.json").read())

# Compare against ALL rods using frequency matching
print(f"\n{'='*60}")
print("FREQUENCY-MATCH COMPARISON (vs all enrolled rods)")
print(f"{'='*60}")
...
for rod_key in sorted(db["rods"].keys()):
    ...
    if not rod.get("enrolled"):
        continue
    enrolled = np.array(rod["perturbed_hz"])
    enr_nonzero = enrolled[enrolled > MIN_FREQ][:10]

    matches = []
    for ef in enr_nonzero:
        closest_idx = np.argmin(np.abs(live_arr - ef))
        closest = live_arr[closest_idx]
        frac = abs(closest - ef) / ef
        matches.append((ef, closest, frac))

    good = [(e, l, f) for e, l, f in matches if f < 0.10]

    print(f"\n  Rod {rod_key}: {len(good)}/{len(matches)} peaks matched within 10%")
    for e, l, f in matches:
        tag = "OK" if f < 0.10 else "MISS"
        print(f"    enrolled {e:8.1f} -> live {l:8.1f}  ({f*100:+6.2f}%)  {tag}")

    if len(good) >= 3:
        fracs = np.array([f for _, _, f in good])
        rms = np.sqrt(np.mean(fracs**2)) * 100
        print(f"    RMS: {rms:.2f}%  (threshold: 2.5%)")
    else:
        print(f"    Too few matches for RMS")
