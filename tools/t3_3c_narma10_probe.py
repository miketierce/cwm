#!/usr/bin/env python3
"""
T3.3c — NARMA-10 via Drive-Gap-Probe Protocol

Key insight from T3.3b failure: continuous driving gives input_corr=0.999
because steady-state dominates. The plate provides NO temporal memory
when driven continuously.

FIX: Separate WRITE and READ phases within each time step.

Protocol per NARMA step t:
  1. WRITE: Drive at u(t)×amplitude for T_WRITE ms (builds mode energy)
  2. GAP:   Stop AWG — USB latency (~5-10 ms) provides natural decay gap
  3. PROBE: Restart AWG at fixed amplitude — captures interference
           between fresh probe and residual from ALL past writes
  4. READ:  Capture amplitude+phase (while probing)

Why this works:
  - T2.1 showed 13.2% contrast with stop→restart (interference IS observable)
  - Ringdown alone is below noise, but INTERFERENCE with probe is detectable
  - Residual encodes: α^1×u(t) + α^2×u(t-1) + ... (exponential memory)
  - Off-resonance phase rotation adds nonlinear mixing: |A_probe + Σ residual_k|
  - α_eff per step ≈ 0.61 × 0.82 (gap decay) ≈ 0.50

Usage:
  python tools/t3_3c_narma10_probe.py [--steps 300] [--offset-hz 5]
"""
...
MODE_FREQ = 35_840.0
AWG_WRITE_UVPP = 500_000   # max write amplitude (0.5 Vpp)
AWG_WRITE_MIN_SCALE = 0.1  # minimum write scaling
AWG_PROBE_UVPP = 500_000   # fixed probe amplitude (max power for acoustic buildup)
T_WRITE_MS = 10            # write duration per step
T_PROBE_MS = 50            # probe settle before capture (long: lets acoustic build to ~87% SS)

TRIGGER_SOURCE = 0
TRIGGER_THRESH = 0
TRIGGER_DIR = 0
TRIGGER_DELAY = 0
TRIGGER_AUTO_MS = 50       # short auto-trigger: memory decays e^{-50/24.5}=13% if real trigger fails
...
def stop_awg(handle, ps2000):
    """Stop AWG by setting amplitude to 0."""
    ps2000.ps2000_set_sig_gen_built_in(
...
def run_narma10_probe(n_steps: int = 300, offset_hz: float = 5.0):
    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    drive_freq = MODE_FREQ + offset_hz

    # Theoretical predictions
    tau_ms = 24.5
    alpha_write = np.exp(-T_WRITE_MS / tau_ms)
    gap_ms_est = 10.0  # estimated USB latency for stop+restart
    alpha_gap = np.exp(-gap_ms_est / tau_ms)
    alpha_total = alpha_write * alpha_gap
    delta_phi_step = 360.0 * offset_hz * (T_WRITE_MS + gap_ms_est + T_PROBE_MS) / 1000.0

    print("=" * 70)
    print("T3.3c — NARMA-10 Drive-Gap-Probe Protocol")
    print("=" * 70)
    print(f"  Mode: {MODE_FREQ:.0f} Hz (Q≈2759, τ≈{tau_ms:.1f} ms)")
    print(f"  Drive freq: {drive_freq:.0f} Hz (Δf = {offset_hz:+.1f} Hz)")
    print(f"  Write: {T_WRITE_MS} ms at u(t)×{AWG_WRITE_UVPP/1e6:.2f} Vpp")
    print(f"  Gap: ~{gap_ms_est:.0f} ms (USB stop→restart latency)")
    print(f"  Probe: {T_PROBE_MS} ms at fixed {AWG_PROBE_UVPP/1e6:.3f} Vpp")
    print(f"  Total step: ~{T_WRITE_MS + gap_ms_est + T_PROBE_MS:.0f} ms")
    print(f"  Decay/step: α_write={alpha_write:.3f}, α_gap={alpha_gap:.3f}, "
          f"α_total={alpha_total:.3f}")
    print(f"  Phase rotation/step: Δφ ≈ {delta_phi_step:.1f}°")
    print(f"  10-step memory: α¹⁰ = {alpha_total**10:.6f}")
    print(f"  Steps: {n_steps} (train: first 60%, test: last 40%)")
    print(f"  Key difference from T3.3b: WRITE and READ are separated by GAP")
    print("=" * 70)
    ...
    print("\n  Collecting reservoir states (drive-gap-probe)...")
    ...
    for t in range(n_steps):
        ...
        # 1. WRITE: drive at input-proportional amplitude
        scale = AWG_WRITE_MIN_SCALE + (1.0 - AWG_WRITE_MIN_SCALE) * (u[t] / 0.5)
        write_uvpp = int(AWG_WRITE_UVPP * scale)
        set_awg(handle, ps2000, drive_freq, write_uvpp)
        time.sleep(T_WRITE_MS / 1000.0)

        # 2. GAP: stop AWG (USB round-trip provides ~5-10 ms natural gap)
        stop_awg(handle, ps2000)

        # 3. PROBE: restart at fixed amplitude
        set_awg(handle, ps2000, drive_freq, AWG_PROBE_UVPP)
        time.sleep(T_PROBE_MS / 1000.0)

        # 4. READ: capture while probing
        mv = capture_triggered(handle, ps2000)
        amp, ph = extract_amplitude_phase(mv, drive_freq)
        amplitudes[t] = amp
        phases[t] = ph

        step_times[t] = (time.perf_counter() - t0) * 1000.0

        if (t + 1) % 50 == 0:
            avg_step = step_times[max(0, t-49):t+1].mean()
            print(f"    [{t+1}/{n_steps}] amp={amp:.0f}, phase={ph:.2f} rad, "
                  f"step={avg_step:.1f} ms")
    ...
    avg_step_ms = step_times.mean()
    print(f"\n  Average step time: {avg_step_ms:.1f} ms "
          f"(total: {step_times.sum()/1000:.1f} s)")
    ...
    # Add polynomial cross-terms (degree 2 on amplitudes)
    amp_cols = X_norm[:, ::2]
    cross_terms = []
    for i in range(min(amp_cols.shape[1], 5)):
        for j in range(i, min(amp_cols.shape[1], 5)):
    ...
    # Plate readouts
    y_hat_train_lin, y_hat_test_lin, _ = ridge_fit_predict(X_train, y_train, X_test)
    y_hat_train_poly, y_hat_test_poly, _ = ridge_fit_predict(
        X_train_poly, y_train, X_test_poly)

    # --- Baseline: input-only features ---
    ...
    corr_amp_input = np.corrcoef(u, amplitudes)[0, 1]
    # Also check correlation with PREVIOUS input (memory indicator)
    corr_amp_prev1 = np.corrcoef(u[:-1], amplitudes[1:])[0, 1]
    corr_amp_prev2 = np.corrcoef(u[:-2], amplitudes[2:])[0, 1]
    corr_amp_prev5 = np.corrcoef(u[:-5], amplitudes[5:])[0, 1]

    # --- Results ---
    print("\n" + "=" * 70)
    print("RESULTS — NARMA-10 (Drive-Gap-Probe)")
    print("=" * 70)
    ...
    if best_nrmse < best_baseline:
        ...
        print(f"  No reservoir improvement over baseline")

    print(f"\n  Amplitude statistics:")
    print(f"    Mean: {amplitudes.mean():.0f}, Std: {amplitudes.std():.0f}")
    print(f"    Min: {amplitudes.min():.0f}, Max: {amplitudes.max():.0f}")

    print(f"\n  Memory diagnostic (amplitude correlations):")
    print(f"    corr(u(t), amp(t)):     {corr_amp_input:.4f}  ← current input")
    print(f"    corr(u(t-1), amp(t)):   {corr_amp_prev1:.4f}  ← 1-step memory")
    print(f"    corr(u(t-2), amp(t)):   {corr_amp_prev2:.4f}  ← 2-step memory")
    print(f"    corr(u(t-5), amp(t)):   {corr_amp_prev5:.4f}  ← 5-step memory")
    print(f"    (T3.3b had corr(u(t), amp(t))=0.999 — pure passthrough)")

    # If we see significant prev-step correlations, the plate has memory
    has_memory = abs(corr_amp_prev1) > 0.1 or abs(corr_amp_prev2) > 0.05
    ...
    if gate == "PASS":
        ...
        print("    Plate shows temporal memory but insufficient for NARMA-10.")
        print("    Consider: more steps, different offset, or multi-mode approach.")
    else:
        print("    No temporal memory detected in probe signal.")

    # Save results
    out_dir = Path("data/results/reservoir_classify")
    out_dir.mkdir(parents=True, exist_ok=True)
    out_file = out_dir / f"t3_3c_narma10_probe_{ts}.json"
    results = {
        "experiment": "T3.3c",
        "protocol": "drive-gap-probe",
        "timestamp": ts,
        "gate_decision": gate,
        "mode_hz": MODE_FREQ,
        "offset_hz": offset_hz,
        ...
        "corr_input_current": float(corr_amp_input),
        "corr_input_lag1": float(corr_amp_prev1),
        "corr_input_lag2": float(corr_amp_prev2),
        "corr_input_lag5": float(corr_amp_prev5),
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="T3.3c NARMA-10 Drive-Gap-Probe")
    ...
    run_narma10_probe(n_steps=args.steps, offset_hz=args.offset_hz)
